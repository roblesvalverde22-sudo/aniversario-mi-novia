...
// 2. Modal de selección de canción
function showAudioModal() {
  const modal = document.createElement("div");
  modal.id = "audio-modal";
  modal.style = "position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);display:flex;align-items:center;justify-content:center;z-index:10;";
  const content = document.createElement("div");
  content.style = "background:#fff;padding:2rem;border-radius:8px;text-align:center;max-width:90%;";
  ...
  tracks.forEach(t => {
    ...
    div.style = "display:inline-block;margin:1rem;cursor:pointer;";
    div.innerHTML = `<img src="${encodeURI(t.cover)}" alt="${t.title}" style="width:100px;border-radius:8px;"/><p>${t.title}</p>`;
...
function typeScroll() {
  ...
  fetch("content/carta-scroll.txt").then(r => r.text()).then(text => {
    cont.textContent = "";
...
// 6. Intentar play + fallback touchstart
...
document.addEventListener("DOMContentLoaded", () => {
  // 7.1 Detección de GitHub Pages vs local
  const isGithub = window.location.hostname.includes("github.io");
  if (isGithub) {
    const bg = document.getElementById("background-video");
    if (bg) { bg.src = "https://github.com/roblesvalverde22-sudo/aniversario-mi-novia/releases/download/v1.0/Particles.Fire.Sparks.mp4"; bg.load(); }
    const overlay = document.getElementById("overlay-video");
    if (overlay) { overlay.src = "https://github.com/roblesvalverde22-sudo/aniversario-mi-novia/releases/download/v1.0/sorpresa.mp4"; overlay.load(); }
  ...
  // 7.9 Animaciones al scroll
  animarAlScroll("#video-especial, #verso-final");

  // 7.10 Interplay vídeo/audio
